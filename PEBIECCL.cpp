/* Plane Extraction, Blurring, and Intrusion/Extrustion Clustering 
 * for Camera Localization
 *
 * PEBIECCL.cpp
 * Written by Kevin Zhang
 * 
 * Input: 1 raw PCLPointCloud2 (.pcd) file in an environment
 *        and 1 PCLPointCloud2 (.pcd) file of the environment
 * Current Output: A visualization of correspondences between the pointclouds 
 *                 to test the strength of correspondences between features.
 * Planned Output: A Point indicating where the picture was taken within the
 *                 3d model of the environment 
 *
 * Note: Still Experimenting with different techniques to improve the accuracy
 *       of feature matching keypoints
 */
...
#include "planeInfo.cpp"
#include "planeObject.cpp"

int mode(std::vector<int> & colorValues); 
void compute_surface_normals (pcl::PointCloud<pcl::PointXYZRGB>::Ptr &points, float normal_radius,
                         pcl::PointCloud<pcl::Normal>::Ptr &normals_out); 
void find_feature_correspondences (pcl::PointCloud<pcl::SHOT1344>::Ptr &source_descriptors,
...
/* 
 * main - Takes in two pointcloud arguments and visualizes the best correspondences
 *        between them
 */
int main (int argc, char** argv)
{
  ...
  if (filenames.size () != 2)
  {
    std::cout << "Filenames missing. Please enter your scene and model pcd filenames.\n";
    exit (-1);
  }
  ...
  /* Plane Extraction and Tagging */ 
  int num_filtered_points = (int) cloud_f->points.size(); //number of points in the filtered cloud
  int i = 0;                                              //counter for the number of planes
  std::vector<planeInfo> pI;                              //storage for individual plane info for plane matching

  while(true) //(cloud_f->points.size() > 0.33 * num_filtered_points)
  {
    try
    {
      ...
      //Changes the direction of the planar pointcloud and saves it to planeCloud which will contain all of the planar clouds
      ...
      // Perform Euclidean Cluster Extraction on the planar surface to get rid of outliers
      ...
      // Uncomment to save the largest cluster in the plane to a point cloud file
      ...
      //Stores the largest cluster in the plane to the planeClouds array
      ...
      // Calculate the average red, green, and blue values of the plane
      int numPlanePoints = planeClouds[i]->points.size();
      int redValues [numPlanePoints];
      int greenValues [numPlanePoints];
      int blueValues [numPlanePoints];

      for(int h = 0; h < numPlanePoints; h++)
      {
        redValues.push_back(planeClouds[i]->points[h].r); 
        greenValues.push_back(planeClouds[i]->points[h].g); 
        blueValues.push_back(planeClouds[i]->points[h].b); 
      }
      ...
      int avgRed = mode(redValues);
      int avgGreen = mode(greenValues);
      int avgBlue = mode(blueValues);

      std::cout << "avgRed =" << avgRed << " avgGreen =" << avgGreen << " avgBlue =" << avgBlue << std::endl; 

      std::vector<planeObject> planeObjects;
      int numPlaneObjects = 0;

      for(int k = 0; k < numPlanePoints; k++)
      {
        if(abs(planeClouds[i]->points[k].r - avgRed) > 3 || abs(planeClouds[i]->points[k].g - avgGreen) > 3 || abs(planeClouds[i]->points[k].b - avgBlue) > 3)
        {
          bool foundObject = false;
          for(int j = 0; j < numPlaneObjects; j++)
          {
            if(abs(planeClouds[i]->points[k].r - planeObjects[j].getAvgRed()) < 3 && abs(planeClouds[i]->points[k].g - planeObjects[j].getAvgGreen()) < 3 && abs(planeClouds[i]->points[k].b - planeObjects[j].getAvgBlue()) < 3)
            {
              pcl::PointCloud<pcl::PointXYZRGB>::Ptr tempPointer(planeObjects[j].getObjectPoints());
              tempPointer->points.push_back(planeClouds[i]->points[k]);
              planeObjects[j].setObjectPoints(*tempPointer);
              foundObject = true;
            }
          }
          // If an object is not found with matching colors, create a new object that contains the one point and has the same color as the point.
          if(!foundObject)
          {
            planeObject pO;
            pcl::PointCloud<pcl::PointXYZRGB> objP;
            objP.push_back(planeClouds[i]->points[k]);
            pO.setAvgObjectColor(planeClouds[i]->points[k].r, planeClouds[i]->points[k].g, planeClouds[i]->points[k].b);
            pO.setObjectPoints(objP);
            pO.setCenter(planeClouds[i]->points[k]);
            planeObjects.push_back(pO);
            numPlaneObjects++;
          }
        } 
      } 
      ...
      for(int j = 0; j < numPlaneObjects; j++)
      {
        ...
        pcl::search::KdTree<pcl::PointXYZRGB>::Ptr ptree (new pcl::search::KdTree<pcl::PointXYZRGB>);
...
/* 
 * mode - returns the mode of a vector of colorValues 
 */
int mode(std::vector<int> & colorValues)
{
  int mostCommonElement = colorValues[0];
  int numOfMostCommon = 0;
  int currentElement = colorValues[0];
  int numOfCurrentElement = 0;
  for (int count = 0; count < colorValues.size(); count++)
  {
    if(colorValues[count] == numOfCurrentElement)
    {
      numOfCurrentElement++;
    }
    else
    {
      if(numOfCurrentElement > numOfMostCommon)
      {
        mostCommonElement = currentElement;
        numOfMostCommon = numOfCurrentElement;
      }
      currentElement = colorValues[count];
      numOfCurrentElement = 1;
    }
  }
...
/* 
 * compute_surface_normals - uses a FLANN-based KdTree to perform neighborhood searches
 *                           and then compute the surface normals
 */
void compute_surface_normals (pcl::PointCloud<pcl::PointXYZRGB>::Ptr &points, float normal_radius,
                         pcl::PointCloud<pcl::Normal>::Ptr &normals_out)
{
...
/* 
 * find_feature_correspondences - Uses a KdTree to check for matches in features
 */
void find_feature_correspondences (pcl::PointCloud<pcl::SHOT1344>::Ptr &source_descriptors,
                              pcl::PointCloud<pcl::SHOT1344>::Ptr &target_descriptors,
                              std::vector<int> &correspondences_out, std::vector<float> &correspondence_scores_out)
...
/* 
 * filter_correspondences - filter correspondences by calculating euclidean distances and 
 *                          keeping those that are below the median
 */
void filter_correspondences(const pcl::PointCloud<pcl::PointWithScale>::Ptr keypoints1,
                            const pcl::PointCloud<pcl::PointWithScale>::Ptr keypoints2,
                            const std::vector<int> &correspondences,
...
/* 
 * visualize_keypoints - given a pointcloud and keypoints, opens a pointcloud visualizer
 */
void visualize_keypoints (const pcl::PointCloud<pcl::PointXYZRGB>::Ptr points,
                          const pcl::PointCloud<pcl::PointXYZRGB>::Ptr keypoints)
{
...
/* 
 * visualize_correspondences - given a pointcloud and correspondences, opens a pointcloud visualizer
 */
void visualize_correspondences (const pcl::PointCloud<pcl::PointXYZRGB>::Ptr points1,
                                const pcl::PointCloud<pcl::PointWithScale>::Ptr keypoints1,
                                const pcl::PointCloud<pcl::PointXYZRGB>::Ptr points2,
